...
def blotter(fname: str,
            pstart: datetime, 
            pend: datetime, aumprimus, aumucits) -> tuple:
    """ Function that generate all the trades executed 
    in a period of time for a specific account
...
def run():
    ...
    data = False
    if(args.blotter):
        data = blotter(args.blotter[0], args.blotter[1], args.blotter[2], args.blotter[3])

    fpath = 'file\path\output.txt'
    f1 = open(fpath,'w')
